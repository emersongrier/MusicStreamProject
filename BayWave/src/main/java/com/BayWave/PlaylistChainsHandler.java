...
class PlaylistChainsHandler implements HttpHandler
{
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        String[] playlistchains;

        try {
            playlistchains = ChainTable.getTableForPlaylist(connection, Integer.parseInt(id));
        } catch (SQLException e) {
            System.err.println("Could not get user info: " + e.getMessage());
            return;
        }

        if (playlistchains == null) {
            exchange.sendResponseHeaders(404, -1);
            return;
        }

        Gson gson = new Gson();
        String json = gson.toJson(playlistchains);
