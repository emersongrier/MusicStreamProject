...
class ArtistDataHandler implements HttpHandler
{
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        try {
            artistinfo = ArtistTable.getArtist(connection, Integer.parseInt(id));
        } catch (SQLException e) {
        ...
        ArtistData artistData = new ArtistData(
                Integer.parseInt(artistinfo[0]),
                artistinfo[1],
                artistinfo[2],
                Integer.parseInt(artistinfo[3]),
                Integer.parseInt(artistinfo[4]));

        Gson gson = new Gson();
        String json = gson.toJson(artistData);
